#
#	Top-level Makefile for Point EMission Addition Software PEmAS
#
#	Macros, these should be generic for all machines
#  
#
#  Created by Agustin on 08/11/2012.
#            version 1.0 12/08/2016
#
#  PURPOSE:  Reads point emissions from a especific File and locates into the mesh
#            of a WRF-chem emissions file.
#
#****************************************************************************
#
...
RM	=	/bin/rm -f
RM_LIST	=	*.mod *.o *.f core .tmpfile pemas.exe ../pemas.exe
...
default:
	...
	if [ $$? = 0 ]; then echo "Compiling for Linux" ;		\
		...
		"FC		= ifort"	"FCFLAGS	= -D$(MACH) -I. -I$(NETCDF)/include -convert big_endian -FR"\
		"LDOPTIONS	= -convert big_endian "	"CFLAGS		= -I."		\
		"LOCAL_LIBRARIES= -L$(NETCDF)/lib -lnetcdff " ) ; \
	else \
		grep Darwin .tmpfile ; \
	if [ $$? = 0 ]; then echo "Compiling for Darwin" ;		\
		...
		"CFLAGS		= -I."		\
		"LOCAL_LIBRARIES= -lnetcdff -lnetcdf -lz -lcurl " ) ; \
